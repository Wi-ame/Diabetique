            ...
            if (email.isNotEmpty()) {
                ...
                databaseReference.orderByChild("email").equalTo(email).addListenerForSingleValueEvent(object :
                    ...
                    override fun onCancelled(databaseError: DatabaseError) {
                        // Gérer l'annulation
                        Toast.makeText(this@Pat_Auth, "Erreur : ${databaseError.message}", Toast.LENGTH_SHORT).show()
                    }
                })
            } else {
                // Champ email vide
                Toast.makeText(this@Pat_Auth, "Veuillez entrer votre email", Toast.LENGTH_SHORT).show()
            }
        }
    }
